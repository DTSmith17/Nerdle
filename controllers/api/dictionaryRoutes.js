...
router.put("/completed/:word_id", async (req, res) => {
  try {
    const userData = await User.findByPk(req.session.user_id);
    // Split existing word IDs by comma and convert them to an array
    const completedWords = userData.word_id ? userData.word_id.split(",") : [];
    // Add the new word's ID to the array of completed words
    completedWords.push(req.params.word_id);
    // Join the array of completed word IDs back into a string, separated by comma
    const updatedUser = await User.update(
      { word_id: completedWords.join(",") },
